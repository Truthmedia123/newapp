...
// Use a function to get the URL to avoid issues with environment variables
const getDirectusUrl = () => {
  if (typeof window !== 'undefined' && window.location) {
    // Client-side - check for environment variable or use default
    return (window as any).DIRECTUS_URL || process.env.REACT_APP_DIRECTUS_URL || 'http://localhost:8055';
  }
